...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss_ce = (
            sigmoid_focal_loss(
                src_logits, target_classes_onehot, num_boxes, alpha=self.focal_alpha, gamma=self.focal_gamma
            )
            * src_logits.shape[1]
        )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_giou = 1 - torch.diag(
            generalized_box_iou(box_cl_to_xy(src_boxes), box_cl_to_xy(target_boxes))
        )
        losses["loss_giou"] = loss_giou.sum() / num_boxes
        self_iou = torch.triu(
            box_iou(box_cl_to_xy(src_boxes), box_cl_to_xy(src_boxes))[0], diagonal=1
        )
...
def cross_entropy_with_gaussian_mask(inputs, targets, opt, weight):
    ...
    gassian_mu = torch.arange(max_seq_len, device=inputs.device).unsqueeze(0).expand(max_seq_len, max_seq_len).float()
