...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        self, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, cost_alpha=0.25, cost_gamma=2
    ):
        """Creates the matcher
    ...
    def forward(self, outputs, targets, verbose=False, many_to_one=False):
        ...
        with torch.no_grad():
            ...
            m2o_rate = 4
            rl_indices = [
                linear_sum_assignment(torch.cat([c[i]] * m2o_rate, -1)) for i, c in enumerate(C.split(sizes, -1))
            ]
            rl_indices = [
                (torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j % sizes[ii], dtype=torch.int64))
                for ii, (i, j) in enumerate(rl_indices)
            ]
