...
class LSTM_Captioner(nn.Module):
    def __init__(self, param_dict):
        ...
        self.input_encoding_size = param_dict["input_encoding_size"]
        self.rnn_size = param_dict["rnn_size"]
        self.num_layers = param_dict["num_layers"]
        self.drop_prob_lm = param_dict["drop_prob"]
        self.max_caption_len = param_dict["max_caption_len"]
        self.caption_loss
...
class AllImgCore(nn.Module):
    def __init__(self, param_dict):
        ...
        self.num_layers = param_dict["num_layers"]
        self.drop_prob_lm = param_dict["drop_prob"]
        self.att_feat_size = param_dict["clip_context_dim"]

        self.param_dict = param_dict
        self.wordRNN_input_feats_type = param_dict["wordRNN_input_feats_type"]
        self.input_dim = self.decide_input_feats_dim()
        self.rnn = nn.LSTM(
            self.input_encoding_size + self.input_dim,
            self.rnn_size,
            self.num_layers,
            bias=False,
...
class LSTM_Deform_Captioner(nn.Module):
    def __init__(self, param_dict):
        super(LSTM_Deform_Captioner, self).__init__()
        self.param_dict = param_dict
    ...
    def build_loss(self, input, target, mask):
        one_hot = torch.nn.functional.one_hot(target, self.param_dict['vocab_size'] + 1)
        max_len = input.shape[1]
        output = -(one_hot[:, :max_len] * input * mask[:, :max_len, None]).sum(2).sum(1) / (mask.sum(1) + 1e-6)
        return output
    ...
    def forward(self, hs, reference, others, cap_tensor):
        ...
        if n_levels < self.core.param_dict['num_feature_levels']:
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, logprob, state = self.get_logprobs_state(
                it,
                state,
                query,
                reference_points,
        ...
        return torch.stack(outputs, 1), torch.stack(logprobs, 1)

    def get_logprobs_state(
        self, it, state, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, mask
    ):
        xt = self.embed(it)
        output, state = self.core(
            xt, state, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, mask
        )
        logprob = F.log_softmax(self.logit(self.dropout(output)), dim=1)
        return output, logprob, state
    ...
    def sample(self, hs, reference, others, opt={}):
        ...
        for t in range(self.max_caption_len + 1):
            if t == 0:  # input <bos>
                ...
                it = torch.multinomial(prob_prev, 1)
                sampleLogprobs = logprob.gather(1, it)  # gather the logprobs at sampled positions
                it = it.view(-1).long()  # and flatten indices for downstream processing
        ...
        if seq == [] or len(seq) == 0:
            return [], []
        return torch.stack(outputs, 1), torch.stack(seq, 1), torch.stack(seqLogprobs, 1)
...
class ShowAttendTellCore(nn.Module):
    def __init__(self, param_dict):
        ...
        self.att_feat_size = int(param_dict["clip_context_dim"] / param_dict["cap_nheads"])
        self.att_hid_size = param_dict["att_hid_size"]

        self.param_dict = param_dict
        self.wordRNN_input_feats_type = param_dict["wordRNN_input_feats_type"]
        self.input_dim = param_dict["hidden_dim"] * 2
        ...
        d_model = param_dict["hidden_dim"]
        self.n_levels = param_dict["cap_num_feature_levels"]
        self.n_heads = param_dict["cap_nheads"]
        self.n_points = param_dict["cap_dec_n_points"]
...
class LSTMDSACaptioner(LSTM_Deform_Captioner):
    def __init__(self, param_dict):
        super(LSTMDSACaptioner, self).__init__(param_dict)
        self.core = ShowAttendTellCore(param_dict)